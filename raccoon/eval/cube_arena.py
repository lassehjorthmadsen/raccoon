...
def gnubg_cube_arena(
    ...
    """Play ``games`` cubeful net-vs-GNUBG money games.
    ...
    ``best_move`` segfaults deeper); ``gnubg_ply`` is the opponent's strength and
    is unconstrained. Keeping them independent is what keeps the estimator
    unbiased whatever the opponent does.
    ...
    from raccoon.eval.gnubg_adapter import (
        board_from_view, gnubg_cube_action, pick_move_cubeful,
    )
    from raccoon.train.lookahead import child_cubeful_values, net_cube_action
    ...
    for g in range(games):
        ...
        while not state.is_terminal() and moves < max_moves:
            if state.is_chance_node():
                ...
                if not is_opening:
                    ...
                    if cube.may_double(mover):
                        label = cube.label_for(mover)   # CENTERED or PLAYER
                        x = x_for(probe.board_from_perspective())
                        board = board_from_view(probe.board_from_perspective())

                        def ask(seat: int) -> tuple[bool, bool]:
                            if seat == net_player:
                                return net_cube_action(
                                    probe._state, net, device, label, x, jacoby,
                                )
                            return gnubg_cube_action(board, gnubg_ply, label, x, jacoby)
            ...
            x = x_for(state.board_from_perspective())
            if mover == net_player:
                legal, cv_values, _ = child_cubeful_values(
                    state._state, net, device, label, x, jacoby, joint_doubles,
                )
                action = legal[int(np.argmax(cv_values))]
            else:
                action = pick_move_cubeful(state, gnubg_ply, label, x, jacoby)
