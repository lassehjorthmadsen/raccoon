...
The honest boundary: this is **parity with 0-ply, not past it**, and 0-ply is a weaker reference than the 2-ply benchmark the rest of this page uses (external estimate: 0-ply ~0.010 ppg from perfect, 2-ply ~0.003). Whether arm A's now-calibrated value head translates to the 2-ply benchmark — directly, or by finally *benefiting* from search where every prior net was hurt by it ([v5](#v5)) — is the open question it hands forward. The methodological result stands regardless: **more clean labels break the plateau**, which turns the scale-up (fleet 0-ply generation + GPU training) into the clear next lever.
...
- **Scale-up.** exp011 established that clean-label *volume* is the lever. The bottleneck then shifts: on CPU, 0-ply generation already outpaces training ~3×, so a serious volume run wants **GPU training fed by fleet 0-ply generation** (the two scale together). The generator needs per-machine shard-name namespacing + seed offsets before multi-machine use.
