...
- **exp011 — clone GNUBG-0-ply's value at volume (planned).** Two independent methods now plateau ~0.3 ppg short of even 0-ply (exp009 DAgger, [exp010](training_analysis.qmd#exp010) TD self-play), and exp010 showed the distilled nets play 0-ply *badly* — their value isn't accurate at the margin between sibling moves. exp011 tests whether **supervised value distillation from GNUBG-0-ply, at ~10M positions, yields a value net accurate enough to play 0-ply at parity with GNUBG-0-ply** — i.e. is the plateau *volume/fidelity-bound* (it clears) or *capacity-bound* (it also stalls). If 0-ply ≈ 2-ply (the external estimate: ~0.010 vs ~0.003 from perfect), 0-ply parity ≈ near-GNUBG, so the payoff is large. Design — local (~48 h) and value-only (the value head is the whole player: 0-ply move selection, no policy head):
  - **Positions from GNUBG-0-ply self-play** (with a little temperature for coverage). Play *is* the label at ~0.003 s/decision → ~10M in ~8 h; the learner-generating *DAgger* variant is too slow to hit 10M in budget (~20 pos/s), and teacher-distribution suffices for cloning a value *function* (on-distribution is a policy-imitation concern; a cheap DAgger refinement can follow).
  - **Confound-free two-arm A/B**, same positions, both from random init (symmetric — no warm start): **arm A** scalar head, MSE against equity; **arm B** six-outcome softmax (win/gammon/bg × win/lose, from GNUBG's cumulative `win,wg,wbg,lg,lbg`), equity derived for play. Arm B is the multi-component target GNUBG/TD-Gammon use, needed eventually for the cube, and it transfers to a TD value head. One oracle call gives both targets.
  - **Eval** each arm's 0-ply play vs GNUBG-0-ply (target ~0). Reaching parity → the plateau was fidelity-bound and we have a strong 0-ply net; *then* price GNUBG-0-ply-vs-2-ply and optionally resume TD self-play from the 0-ply-parity seed (exp010 was crippled starting at −2.30). Stalling at ~−0.3 too → capacity/architecture bound.
  - Training two 10×256 nets on ~10M positions on the iMac CPU is the tight part of the 48 h; positions vs epochs is the knob to fit.
