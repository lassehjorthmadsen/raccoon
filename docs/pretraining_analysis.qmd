...
- **How much is left in epochs and labels? [exp017](#exp017) stopped on a wall-clock cap, not on convergence.** PR was still descending at −0.0082/epoch over ep6–14 (t = −4.0) when the 72 h budget ran out, while rollout MSE had been flat since ~ep8 — so the run answers "is 40M better than 8M" but not "is 40M enough". Two concrete follow-ups, in cost order. **(i) More epochs, ~free to set up:** `train_distill.py --resume auto` restarts from `latest.pt` with optimizer state intact, so continuing ep15→ep24 on the existing corpus is one relaunch at ~4.5 h/epoch on a T4 spot VM; a power-law fit predicts ≈0.88 by ep20, and it costs nothing in new labels. **(ii) More labels:** the data axis has exactly two points (8M, 40M), which cannot distinguish a power law from a saturating curve — extrapolation puts 80M at ≈0.76 (floored at the teacher's 0.56) or ≈0.65 (unfloored), both material at this end of the scale, but neither is evidence. Generating another 40M is days of GNUBG self-play; a cheaper intermediate is to train matched-epoch runs on 16M and 24M *subsets of the corpus we already have*, which costs only GPU time and turns two points into four — enough to fit the exponent and predict 80M before paying to generate it. Confounded with all of this: exp014's 8M anchor ran 3 epochs to exp017's 12, so no existing pair separates data volume from optimisation budget; the 16M/24M subset runs would fix that too, if held to a matched epoch count.
