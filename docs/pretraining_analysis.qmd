...
# as four n=50 text chunks.
#
# DO NOT also take these from round_15/logs/gnubg_eval_log.jsonl. The GPU VM's
# copy of that jsonl holds the same four runs (verified identical: 5-45/-1.58,
# 5-45/-1.46, 7-43/-1.30, 9-41/-1.16), because the eval wrote both formats there.
# The committed jsonl deliberately excludes them, so syncing the VM's version over
# it would count this benchmark twice and shift the exp008 bar.
...
```{python}
#| echo: false
# exp018 per-epoch BGSage results. Reuses REF / PLANS / _bench_load / _move_row /
# _acc_row from the exp017 chunks above (same kernel, same benchmark, same n), and
# keeps its own EPOCHS18 so exp017's EPOCHS is left intact.
EXP018_RES = EXP_ROOT / "exp018-benchmark" / "results"
N_EPOCHS_18 = 24

EPOCHS18 = {}
if EXP018_RES.exists():
    for p in EXP018_RES.glob("*.json"):
        if p.name == "summary.json":
            continue
        d = _bench_load(p)
        m = re.match(r"exp018-ep(\d+)$", d.get("engine_label", ""))
        if m:
            EPOCHS18[int(m.group(1))] = d

EXP018_READY = len(EPOCHS18) >= N_EPOCHS_18
BEST18 = min(EPOCHS18, key=lambda e: EPOCHS18[e]["pr"]) if EPOCHS18 else None
LAST18 = max(EPOCHS18) if EPOCHS18 else None
BEST18_MSE = (min(EPOCHS18, key=lambda e: EPOCHS18[e]["eval_accuracy"]["rollout"]["mse"])
              if EPOCHS18 else None)

# Plateau level vs the pre-registered Q2 prediction. The benchmark is a fixed,
# deterministically-scored set, so this scatter is real checkpoint-to-checkpoint
# variation, not sampling noise.
def _pr_stats(d, lo, hi):
    v = [d[k]["pr"] for k in d if lo <= k <= hi]
    return (np.mean(v), np.std(v, ddof=1), len(v)) if v else (np.nan, np.nan, 0)

PLATEAU18 = _pr_stats(EPOCHS18, 15, 24)      # after the descent stops
LATE17 = _pr_stats(EPOCHS, 12, 14)           # exp017's last three epochs
MID18 = _pr_stats(EPOCHS18, 12, 14)          # exp018 at exp017's endpoint
```

*Move selection* — PR = mean(error) × 500 over 14,693 checker decisions (lower is better), with the per-game-plan breakdown; reference rows carried from [exp015](#exp015) and [exp017](#exp017):

```{python}
#| echo: false
#| output: asis
if not EXP018_READY:
    print(f"*exp018 scoring in progress — {len(EPOCHS18)}/{N_EPOCHS_18} epochs scored; "
          f"tables and figure auto-populate on re-render once "
          f"`experiments/score_exp018.sh` finishes.*")
else:
    rows = [_move_row(n, REF[n]) for n in REF_ORDER]
    rows.append(_move_row(f"exp017 ep{BEST_EP} (scalar, best PR)", EPOCHS[BEST_EP]))
    rows.append(_move_row(f"exp018 ep{BEST18} (outcomes6, best PR)", EPOCHS18[BEST18]))
    rows.append(_move_row(f"exp018 ep{LAST18} (outcomes6, last)", EPOCHS18[LAST18]))
    print(pd.DataFrame(rows).to_markdown(index=False))
```
...
if EXP018_READY:
    rows = [_acc_row(n, REF[n]) for n in REF_ORDER]
    rows.append(_acc_row(f"exp017 ep{BEST_EP} (scalar, best PR)", EPOCHS[BEST_EP]))
    rows.append(_acc_row(f"exp018 ep{BEST18_MSE} (outcomes6, best MSE)", EPOCHS18[BEST18_MSE]))
    rows.append(_acc_row(f"exp018 ep{LAST18} (outcomes6, last)", EPOCHS18[LAST18]))
    print(pd.DataFrame(rows).to_markdown(index=False))
```
...
if EPOCHS18:
    rows = []
    for ep in sorted(EPOCHS18):
        d = EPOCHS18[ep]
        row = {"epoch": ep, "PR (outcomes6)": round(d["pr"], 3)}
        row["PR (scalar)"] = round(EPOCHS[ep]["pr"], 3) if ep in EPOCHS else None
        row.update({pl: round(d["by_plan"][pl]["pr"], 2) for pl in PLANS})
        row["rollout MSE"] = round(d["eval_accuracy"]["rollout"]["mse"], 5)
        rows.append(row)
    print(pd.DataFrame(rows).to_markdown(index=False))
```
...
#| echo: false
if EXP018_READY:
    eps18 = sorted(EPOCHS18)
    eps17 = sorted(EPOCHS)
    PLAN_COLORS = {"purerace": "#D55E00", "racing": "#0072B2", "attacking": "#009E73",
                   "priming": "#CC79A7", "anchoring": "#E69F00"}
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(8.5, 10.4), sharex=True)

    # Panel 1: the two heads on identical data/order — Q1 is vertical distance,
    # Q2 is the shape past ep14.
    ax1.plot(eps18, [EPOCHS18[e]["pr"] for e in eps18], "-o",
             color="#333333", lw=2.4, ms=6, label="exp018 outcomes6 (24 ep)")
    ax1.plot(eps17, [EPOCHS[e]["pr"] for e in eps17], "-s",
             color="#0072B2", lw=2.0, ms=5, label="exp017 scalar (14 ep, wall cap)")
    if not np.isnan(PLATEAU18[0]):
        m, s, _ = PLATEAU18
        ax1.axhspan(m - s, m + s, xmin=0.55, color="#333333", alpha=0.10)
        ax1.axhline(m, ls="--", lw=1.2, color="#333333")
        ax1.text(eps18[-1], m, f" ep15–24 mean {m:.3f}", va="bottom", ha="right",
                 fontsize=8, color="#333333")
    ax1.axvline(14, ls=":", lw=1.2, color="#888888")
    for name, col in [("GNUBG 2-ply", "#009E73"), ("GNUBG 0-ply", "#CC79A7")]:
        if name in REF:
            ax1.axhline(REF[name]["pr"], ls="-.", lw=1.3, color=col,
                        label=f"{name} ({REF[name]['pr']:.2f})")
    # Headroom above the 0-ply reference so the legend never sits on a line.
    ax1.set_ylim(top=max(REF["GNUBG 0-ply"]["pr"], 2.2) * 1.30)
    ax1.text(14, ax1.get_ylim()[1], " exp017 cap", va="top", ha="left",
             fontsize=8, color="#888888")
    ax1.set_ylabel("PR  (↓ better)")
    ax1.set_title("Move-selection PR: six-outcome vs scalar head on identical shards")
    ax1.grid(True, alpha=0.3)
    ax1.legend(fontsize=8, loc="upper left", framealpha=0.95)

    # Panel 2: rollout MSE, both heads. Read asymmetrically for Q1 — this is the
    # scalar arm's own training objective.
    ax2.plot(eps18, [EPOCHS18[e]["eval_accuracy"]["rollout"]["mse"] for e in eps18],
             "-o", color="#333333", lw=2.4, ms=6, label="exp018 outcomes6")
    ax2.plot(eps17, [EPOCHS[e]["eval_accuracy"]["rollout"]["mse"] for e in eps17],
             "-s", color="#0072B2", lw=2.0, ms=5, label="exp017 scalar")
    for name, ls, col in [("GNUBG 2-ply", "--", "#009E73"),
                          ("GNUBG 0-ply", "-.", "#CC79A7")]:
        r = REF.get(name)
        if r:
            mse = r["eval_accuracy"]["rollout"]["mse"]
            ax2.axhline(mse, ls=ls, color=col, lw=1.4, label=f"{name} ({mse:.5f})")
    ax2.set_ylabel("rollout MSE  (↓ better)")
    ax2.set_title("Value-fit to the rollout reference vs the teacher")
    ax2.grid(True, alpha=0.3)
    ax2.legend(fontsize=8, loc="best")

    # Panel 3: per-plan PR for exp018 against its own GNUBG-0-ply reference.
    g0 = REF.get("GNUBG 0-ply")
    for pl in PLANS:
        ax3.plot(eps18, [EPOCHS18[e]["by_plan"][pl]["pr"] for e in eps18],
                 "-o", color=PLAN_COLORS[pl], lw=1.8, ms=4, label=pl)
        if g0:
            ax3.axhline(g0["by_plan"][pl]["pr"], ls=":", lw=1.1, color=PLAN_COLORS[pl])
    if g0:
        ax3.plot([], [], ls=":", lw=1.3, color="#666666",
                 label="GNUBG 0-ply (matching colour)")
    ax3.set_ylabel("PR by game plan  (↓ better)")
    ax3.set_title("Per-plan PR against the GNUBG-0-ply reference")
    ax3.grid(True, alpha=0.3)
    # Clear the highest dotted 0-ply reference before placing the legend.
    if g0:
        ax3.set_ylim(top=max(g0["by_plan"][pl]["pr"] for pl in PLANS) * 1.32)
    ax3.legend(ncol=3, fontsize=8, loc="upper center", framealpha=0.95)
    ax3.set_xlabel("epoch")
    ax3.set_xticks(eps18[::2])
    plt.tight_layout()
    plt.show()
```
...
That resolves the *measurement*, not the *attribution*. The bootstrap propagates benchmark sampling error only; it says nothing about **initialisation variance**, and since `train_distill.py` does not seed model init and this is one run per arm, a ~0.067 PR difference between two runs is exactly the size [exp016](#exp016) judged inseparable from init noise (it saw 0.13 between arms it could not separate). So "the six-outcome head costs ~0.07 PR" and "this initialisation drew ~0.07 worse" remain observationally equivalent here. Separating them needs several seeds per arm, which this run deliberately did not buy.
