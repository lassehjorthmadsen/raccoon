...
## Stage 7 — on-distribution distillation / DAgger (`exp008`)

Stages 3–6 showed that archive-based distillation tops out at ~−1.6 ppg vs GNUBG 2-ply regardless of capacity, eval search, or coverage — with the diagnosis pointing squarely at on-distribution *policy* as the binding gap (Learning #9). Meanwhile, [exp007](training_analysis.qmd#exp007-self-play-from-the-v5-supervised-seed) showed that pure self-play from the v5 seed *regresses* it completely (93-7 → 51-49 vs iter_0447 in 50 iterations). Stage 7 tests the fix: **expert iteration** implemented as **offline on-distribution distillation (DAgger)** rather than online replay-buffer mixing. The design attacks both mechanisms behind the regression trap:

- **On-distribution states.** Each round the *current* net plays out games (its own move, temperature-sampled), so training positions are the lines the learner actually reaches — not the human-match archive, which the coverage analysis above identified as the gap behind the residual ~−1.6 ppg.
- **Both targets from the GNUBG oracle.** Every visited position is labeled *live* by `gnubg-nn` at **2-ply**: a soft policy over candidate moves *and* the money-equity value. This replaces MCTS visit counts (which reflect a GNUBG-2-ply-losing player) and discrete terminal outcomes (which pulled the value head off calibration) — the two things that broke exp007. Pure supervised distillation throughout, so the trap cannot recur by construction.
...
1. **The regression trap is decisively avoided** — the point of the experiment. exp007 took v5 to 10-90 / −1.74 ppg vs the seed; exp008 *climbs* to 63-37 / +0.48 by round 15. On-distribution GNUBG DAgger validates the pretraining pivot where naive self-play destroyed it.
2. **Statistically-significant improvement over v5.** The per-round vs-seed climb is noisy but trends up (early ~+0.15–0.29 → late ~+0.48–0.60). Pooling the last four rounds: 245/400 = 61% vs parity (z ≈ 4.4); round 12 alone (65-35) is significant on its own. Arena at 50 sims, symmetric — a fair relative test.
3. **The external GNUBG gap narrowed.** At fixed 50-sim conditions win rate rose 7% → 17% and equity −1.65 → −1.19 over rounds 4 → 15 (two-proportion test on 7% vs 17%: p ≈ 0.03). **−1.19 ppg / 17% is the project's best result vs GNUBG 2-ply.**
...
11. **On-distribution DAgger breaks the ~−1.6 ppg ceiling — the first method to improve on the strongest seed.** Stage 7 replaced the fixed archive with positions from the learner's own play, labeled live by GNUBG 2-ply. Over 15 rounds (200 CPU-hours), the network climbed from v5's −1.59 to **−1.19 ppg / 17% wins** vs GNUBG 2-ply — the project's best external result — and beat its own seed 63-37 (+0.48 ppg). The mechanism validated the exact diagnosis from #9: the archive lacks the positions the learner reaches, and on-dist labeling fills that gap. Pure supervised distillation throughout (no RL signal), so the regression trap from exp007 cannot recur. The ceiling is now teacher quality × round count; reaching parity likely needs 4-ply on-dist labels or more rounds.

## Open questions

This track started as a way to produce a **seed for self-play** (exp007), but exp007's regression trap showed that pure self-play destroys the seed. Stage 7 (DAgger) reframed the track as the *primary* training method: on-distribution supervised distillation from the GNUBG oracle. The figure of merit is now GNUBG-2-ply equity directly, and the open levers are teacher ply, round count, and coverage of the on-dist positions.
...
- **Resolved (2026-06-14, exp007; 2026-07-06, exp008/Stage 7): self-play regresses the seed — but on-distribution DAgger improves it.** exp007 showed pure self-play from v5 regresses completely (93-7 → 51-49 vs iter_0447 in 50 iters; see [Training Analysis](training_analysis.qmd)). Stage 7 above is the fix: on-distribution GNUBG 2-ply DAgger climbs to 63-37 / +0.48 ppg vs the seed and narrows the external gap to −1.19 ppg — the project's best result. The approach validates the diagnosis from #9: the binding gap was on-distribution *policy*, not archive breadth. Reaching GNUBG parity remains open.
