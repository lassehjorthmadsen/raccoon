...
*(Numbers below are transcribed from the run's printed output, not recomputed at render time like the rest of this page — the benchmark file lives in a sibling `bgsage` repo outside this project and the run's raw per-decision JSON wasn't saved (`--output` was not passed), so reproducing the exact table requires re-running the script. Update, 2026-07-31: the benchmark file is now copied into `data/bgsage/money_benchmark/benchmark.json.gz` (sourced from the bgsage repo, cloned as a sibling project at `~/python-projects/bgsage` — see `data/README.md`), so `scripts/eval_benchmark_pr.py` can be re-run directly; this table itself remains the original transcribed run, not re-derived.)*
...
**Interpretation.** Two distilled nets, an external reference, and a static protocol reproduce the R²-based ordering from [exp014](#exp014) with an independent, lower-variance metric: **exp014 (2-ply-distilled) beats exp011b (0-ply-distilled) on both PR (2.51 vs 3.53) and R² (0.9919 vs 0.9831 on the rollout tier)** — the stronger teacher produces the stronger student, confirmed by move selection, not just fit quality. Neither raccoon net matches GNUBG 0-ply's move selection (PR 2.14) despite exp014 training on GNUBG **2-ply** labels; the shortfall is consistent across game plans, and **worst on pure-race positions** (exp014 1.62, exp011b 1.51) — exactly where GNUBG plays nearest-perfectly (0.05–0.16) and where a value function should have the easiest time, an open puzzle called out below.

**Conclusion.** *Does the 2-ply-distilled net actually play better than the 0-ply-distilled net, independent of exp014's R² proxy?* → **Yes**: on a static, external-reference PR benchmark (n=14,693 decisions), exp014 clearly outscores exp011b (PR 2.51 vs 3.53), corroborating exp014's R²-only sign with a genuinely different, move-selection metric. Both remain well short of GNUBG 0-ply (PR 2.14), and the raw-ppg *size* of the gain — like the full-8M deliverable training — is still open.
...
14. **A static, external-reference PR benchmark confirms exp014's R² sign with an independent metric**: the 2-ply-distilled net beats the 0-ply-distilled net on move selection too (PR 2.51 vs 3.53), not just fit quality — but neither matches GNUBG 0-ply (PR 2.14), and both are surprisingly weakest on pure-race positions ([exp015](#exp015)).
...
- **Race positions are the distilled nets' relative weak point.** [exp015](#exp015)'s per-game-plan PR breakdown shows both distilled nets doing relatively worst on pure-race positions (exp014 1.62, exp011b 1.51) versus GNUBG's near-perfect race play (2-ply 0.05, 0-ply 0.16) — the largest relative gap of any game plan, and a surprising one, since races have no contact and should be the easiest category for a value function to get right. Unexplored: whether this is a labeling artifact of the benchmark's race positions specifically, or a genuine weakness in how either distilled net handles pure races.
