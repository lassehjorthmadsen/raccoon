...
**Interpretation.** The pipeline check passes exactly — the exp014 8M anchor re-scores to PR 2.16 / rollout-MSE 0.0044, identical to [exp015](#exp015). Against it, 40M is a transformation, not a nudge: the best epoch (ep12) scores overall **PR 0.93** — down from 2.16, *past* GNUBG-0-ply's own raw 2.14 and most of the way to the GNUBG-2-ply teacher (0.56) — while rollout-tier MSE bottoms at **0.0019** (ep8–11), **matching the 2-ply teacher's 0.0019** and beating GNUBG-0-ply's 0.0021 (8M was 0.0044). So the 8M net was badly *underfit*, exactly as its 2.3× rollout-MSE gap warned; the 8M PR≈GNUBG-0-ply coincidence was never a searchless ceiling. **(a) Plateau**: both benchmark curves fall steeply through ~ep5, then flatten — PR oscillates 0.93–1.01 from ep6 on, MSE sits at 0.0019–0.0021 from ep8 on. The training loss makes that a sharper claim than "the run converged", because it is *still descending* at the cap: 28.5 → 1.5 ×10⁻⁴ overall, and still −25% between ep8 and ep14 (a 15th epoch ran into the 72 h wall and landed within 0.3% of ep14). So optimisation had not stalled — the net went on fitting its 2-ply labels while BGSage error stood still. Label-fit and benchmark strength **decoupled**, which is a firmer reason to stop than a flat loss curve would have been: further epochs on this corpus buy a tighter fit to GNUBG-2-ply's own answers and nothing measurable in play. Consistently, by ep14 the net sits ~12× closer to its training labels (1.5 ×10⁻⁴) than to BGSage's rollout reference (19 ×10⁻⁴) — in-sample against out-of-sample, so not a like-for-like gap, but far too large a ratio for the residual benchmark error to be failure to fit. The remainder is teacher-vs-rollout disagreement plus the 2-ply search the teacher runs *at play time*, not missing labels. **(b) Trade-off**: none — *every* plan improves together (ep12 vs anchor: priming −1.67, anchoring −1.24, racing −1.22, attacking −1.14, purerace −0.84). Pure race — exp015's stark 1.13 outlier (~7× the teacher) — falls to 0.29, so that weakness was a **data/underfitting artifact resolved by scale**, not a structural blind spot. **(c) Agreement**: PR and rollout-MSE improve together and bottom within one epoch of each other (PR at ep12, MSE at ep11) — so unlike [exp015](#exp015)/[exp016](#exp016), the two metrics agree cleanly.

**Conclusion.** *Does scaling 2-ply distillation 8M→40M lower BGSage error and fix pure races without trading off other plans?* → **Yes, decisively — and it plateaus.** Best epoch ep12 = **PR 0.93** (from the 8M anchor's 2.16, below GNUBG-0-ply's own 2.14) with rollout-MSE bottoming at **0.0019** (level with the GNUBG-2-ply teacher, from 0.0044); n=14,693 / n≈149k. All five game plans improve together — pure race 1.13→0.29 — so there is no trade-off and the race anomaly was underfitting, not structure. Both curves flatten by ~ep6–8, so the data/epoch lever is now spent; the gap left to the teacher's 0.56 PR is its play-time 2-ply search, not more labels. (Static-benchmark PR/MSE, not raw ppg — the fair-beat ppg confirmation at matched search is the open finish.)
