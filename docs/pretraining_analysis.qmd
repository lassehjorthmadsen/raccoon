...
17. **Scaling the 2-ply distillation 8M→40M is transformative** — static-benchmark PR falls from 2.16 to 0.93 and rollout-MSE from 0.0044 to 0.0019 (within 2% of the GNUBG-2-ply teacher's 0.00187, past GNUBG-0-ply on both), every game plan improving together with no trade-off, and the pure-race weakness ([exp015](#exp015)) turning out to be an artifact (1.13→0.29). The 8M net was underfit, not capacity-bound — though it ran 3 epochs to the 40M run's 12, so *volume* and *optimisation budget* both changed and this pair alone can't apportion the credit. And the run is **not** converged: rollout MSE flattens by ~ep8 but PR is still descending when the wall-clock cap hits, so more epochs and more labels both remain live levers ([exp017](#exp017)).

## Open questions

- **The strongest net has no cube-capable head — and that is the next blocker, not more data.** [exp017](#exp017)'s deliverable trains the **scalar** value head (`value_fc2` is 1×256), so it emits a cubeless equity and nothing else. Cube decisions need the win/gammon/backgammon split, which only the six-outcome head provides — and the only outcomes6 net we have is [exp011b](#exp011b)'s ep3, back at PR 2.75 against exp017's 0.93. The corpus already solves half of it: the 2-ply shards carry valid `outcomes6` labels alongside `equity` (columns `[win, win-g, win-bg, lose, lose-g, lose-bg]`, verified to sum to 1.0), so an outcomes6 net at 40M needs **no new labels** — the same corpus with `--value-head outcomes6`. That one run does triple duty: it produces the cube-capable deliverable at current best scale, it answers the epoch question above on the arm that actually matters, and — since exp017 supplies a per-epoch *scalar* curve on the identical corpus — it is a properly matched A/B for the value-head comparison [exp016](#exp016) left unresolved. A cheaper first attempt: warm-start from exp017's ep12 trunk with a swapped six-output head and fine-tune, ~⅕ the cost, falling back to from-scratch (which [exp011b](#exp011b) shows works) if its BGSage curve lands below exp017's. Note that outcomes6 is *necessary but not sufficient* for the cube — cubeful equity and take points still sit on top of the distribution.

- **How much is left in epochs and labels? [exp017](#exp017) stopped on a wall-clock cap, not on convergence.** PR was still descending at −0.0082/epoch over ep6–14 (t = −4.0) when the 72 h budget ran out, while rollout MSE had been flat since ~ep8 — so the run answers "is 40M better than 8M" but not "is 40M enough". **On epochs:** `train_distill.py --resume auto` restarts from `latest.pt` with optimizer state intact, so more epochs cost only GPU time at ~4.5 h/epoch on a T4 spot VM, and a power-law fit predicts ≈0.88 by ep20 — but this is better folded into the outcomes6 run above than spent extending an arm that has to be redone anyway. **On labels:** the data axis has exactly two points (8M, 40M), and two points fit any two-parameter curve exactly — so the 80M extrapolations (≈0.76 floored at the teacher's 0.56, ≈0.65 unfloored) come from the assumed form, not from evidence. A ladder of runs on 9M/17M/24M subsets of run3 (verified to share run1's label and position distribution, so they sit on one curve) would measure the *marginal return per doubling* near the current scale — the decision-relevant quantity — though four points across a single decade would not settle the functional form or the asymptote. **Worth doing only if generating more labels is a cost we would be paying ourselves**; if a larger corpus can be sourced instead, the ladder's whole purpose (deciding whether to fund generation) falls away, and the effort belongs on teacher *quality* instead — a 4-ply teacher moves the 0.56 asymptote itself, where more 2-ply labels only slide us along a curve that ends there. Separately, exp014's 8M anchor ran 3 epochs to exp017's 12, so no existing pair separates data volume from optimisation budget; a matched-budget subset ladder would fix that too.
