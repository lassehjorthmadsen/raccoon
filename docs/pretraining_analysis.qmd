...
The same curve as a figure — **top panel**: each game plan's PR *relative to the 8M anchor* (dashed zero line), so below zero is better than 8M and a race fix that costs contact plans shows pure-race dropping while another plan rises above zero; **bottom panel**: rollout value-fit MSE against the GNUBG teacher references.
...
if EPOCHS:
    ...
    # Panel 1: PR change vs the 8M anchor, per plan (+ overall).
    if anchor:
        for pl in PLANS:
            base = anchor["by_plan"][pl]["pr"]
            ax1.plot(eps, [EPOCHS[e]["by_plan"][pl]["pr"] - base for e in eps],
                     "-o", color=PLAN_COLORS[pl], lw=2, ms=6, label=pl)
        ax1.plot(eps, [EPOCHS[e]["pr"] - anchor["pr"] for e in eps],
                 "-o", color="#333333", lw=2.6, ms=7, label="overall")
        ax1.axhline(0, color="#333333", ls="--", lw=1.2)
        ax1.text(eps[0], 0, " exp014 8M anchor", va="bottom", ha="left",
                 fontsize=8, color="#333333")
    ax1.set_ylabel("PR − 8M anchor\n(↓ = better than 8M)")
    ax1.set_title("Move-selection PR by game plan, relative to the 8M anchor")
