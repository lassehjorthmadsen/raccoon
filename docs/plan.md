...
- **Supervised pretraining on labeled positions (GNUBG-tradition)** *(implemented)*: After exp000–exp006 all plateaued, we added value-only supervised pretraining on wildbg-training's ~300k rollout-labeled positions (CC0; `data/0021/contact.csv` + `data/0022/race.csv`). The decoder lives in `raccoon/data/wildbg.py` (GNUBG Position ID → `BoardView`), and `scripts/pretrain.py` regresses the value head with MSE on equity = `((win − lose) + (win_g + win_bg) − (lose_g + lose_bg)) / 3`. The output checkpoint is a drop-in for `train.py --resume`. The policy head receives no direct gradient; it picks up signal via the shared trunk and is refined by subsequent self-play (exp007).
