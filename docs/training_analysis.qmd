...
_rows = [json.loads(l) for l in
         ...
         if l.strip()]
SEED_1PLY = -2.30  # round_06 played 1-ply vs GNUBG-0-ply (diagnostic, n=20)
xs = [0] + [r["batch"] for r in _rows]
ys = [SEED_1PLY] + [r.get("eval_vs_gnubg0ply_ppg") for r in _rows]
fig, ax = plt.subplots(figsize=(8.5, 4.2))
ax.plot(xs, ys, "-o", color="#17becf", ms=4, lw=1.3)
ax.axhline(0, color="k", lw=1.2, label="GNUBG-0-ply parity")
ax.axhline(SEED_1PLY, ls="--", color="#d62728", lw=1.1,
           label=f"round_06 seed at 1-ply ({SEED_1PLY:+.2f})")
ax.set_xlabel("TD batch (150 self-play games each; batch 0 = seed)")
ax.set_ylabel("Equity/game vs GNUBG-0-ply")
ax.set_title("exp010: TD lifts 1-ply value play fast, then plateaus ~0.3 short of 0-ply")
ax.set_ylim(-2.5, 0.3)
...
1. **TD self-play *works* — it improves the seed, where MCTS self-play destroyed it.** The seed plays 1-ply at −2.30 ppg vs GNUBG-0-ply; TD hauls it to ~−0.3 (best-of-batch −0.08) in the first few hundred games. This is the first self-play result on the project to *lift* a strong seed rather than wreck it — the value-only bootstrap from a money-calibrated head is the difference from exp007.
