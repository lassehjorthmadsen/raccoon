...
EXPERIMENTS = {
    ...
    "exp007": {
        "name": "exp007-10x256-v5seed-800sims",
        "label": "exp007 (800 sims, 8 games, 10×256, v5 seed)",
        "color": "#17becf",
        "network": "10×256 ResNet",
    },
}
...
for exp_id, exp in experiments.items():
    ...
    summary_data.append({
        "Experiment": exp["name"],
        "Network": exp.get("network", "6×128 ResNet"),
...
exp007_evals = []
el_path = Path("../experiments/exp007-10x256-v5seed-800sims/logs/eval_log.jsonl")
gl_path = Path("../experiments/exp007-10x256-v5seed-800sims/logs/gnubg_eval_log.jsonl")
if el_path.exists():
    for l in el_path.read_text().splitlines():
        if l.strip():
            r = json.loads(l)
            opp = Path(r.get("opponent","?")).stem
            exp007_evals.append({
                "Opponent": opp.replace("pretrained_v2","v5 seed").replace("iter_0447","iter_0447 (6×128 self-play)"),
                "Result": f"{r['p1_wins']}-{r['p2_wins']}",
                "Win%": f"{r['p1_wins']/r['num_games']*100:.0f}%",
                "Equity": f"{r['equity']:+.3f}",
                "Δ vs seed": f"{r['equity']-1.98:+.2f}" if "pretrained" not in r.get("opponent","") else "—",
                "n": r["num_games"],
            })
if gl_path.exists():
    g = json.loads(gl_path.read_text())
    exp007_evals.append({
        "Opponent": "GNUBG 2-ply",
        "Result": f"{g['raccoon_wins']}-{g['gnubg_wins']}",
        "Win%": f"{g['win_rate']*100:.0f}%",
        "Equity": f"{g['equity_per_game']:+.3f}",
        "Δ vs seed": f"{g['equity_per_game']-(-1.59):+.2f}",
        "n": g["num_games"],
    })
if exp007_evals:
    import pandas as pd
    print(pd.DataFrame(exp007_evals).to_markdown(index=False))
else:
    print("exp007 eval logs not found.")
```

50 iterations of self-play took a network that dominated iter_0447 **93-7 (+1.98 ppg)** to one at **dead parity (51-49, 0.000 ppg)**. Against GNUBG it went from −1.59 to −2.24 ppg — nearly back to where the project started before the pretraining pivot. The network lost 10-90 (−1.74 ppg) to its own starting point. The regression trap was complete.
...
**Fixes.** The root problem is that pure self-play fully discards the supervised signal within ~30 iterations (the 25k buffer saturates at ~800 positions/iter × 31 iters). The solution — on-distribution expert iteration (DAgger) — is documented in [Pretraining Analysis § Stage 7](pretraining_analysis.qmd). Headline result: 15 rounds of on-dist GNUBG-2-ply DAgger climbed to +0.48 ppg vs the v5 seed and narrowed the external gap to **−1.19 ppg / 17% wins** — the project's best result.
...
| v5 supervised seed + pure self-play | exp007 | Complete regression: 10-90 vs seed (−1.74 ppg); parity with iter_0447 (51-49, 0.000). Pure self-play discards the supervised signal — fix is on-dist DAgger (see [Pretraining Analysis § Stage 7](pretraining_analysis.qmd)) |
...
- M6 milestone (money-game wins against GNUBG) requires an algorithmic change, not just more iterations. That change — on-distribution DAgger from the GNUBG oracle — is documented in [Pretraining Analysis § Stage 7](pretraining_analysis.qmd) and produced **−1.19 ppg / 17% wins**, the project's best result.
...
8. ~~**Algorithmic pivot: supervised pretraining + self-play fine-tune.**~~ **Done** (exp007, 2026-06-14; exp008/Stage 7, 2026-07-06). exp007: pure self-play from v5 regresses completely (−1.74 ppg vs seed). The fix — on-distribution DAgger (exp008) — is documented in [Pretraining Analysis § Stage 7](pretraining_analysis.qmd) and produced the project's best result: **−1.19 ppg / 17% wins vs GNUBG 2-ply.**

9. **Post-DAgger direction: TD(λ) self-play from the strong seed (exp010 front-runner, noted 2026-07-10).**
   DAgger (exp008/exp009) converges to its teacher and can never pass it; GNUBG 2-ply is its
   ceiling by construction. Of the ways past that ceiling, TD-Gammon/GNUBG-style TD self-play
   looks strongest on cost and evidence:
   - **Cheap where AlphaZero is expensive.** TD move selection is 1-ply: ~20 value evals per
     decision (one per legal move) vs 100–800 for MCTS self-play, no tree overhead, and the
     candidate positions batch naturally on GPU. The dice provide exploration for free — no
     Dirichlet noise, no temperature schedule. A TD-Gammon-scale diet (~1M self-play games) is
     plausibly a couple of GPU-weeks on the current T4 with the current 10×256 net. The recipe
     the core conclusion above calls too expensive is *MCTS* self-play; TD never was.
   - **No teacher ceiling, warm start.** Initialise the value head from the consolidation seed
     (already distilled from GNUBG) and let TD refine beyond it. The policy head can be
     regenerated afterwards from the improved value function via the existing 1-ply V-lookahead
     distillation (`synthesize_policy_dataset.py`).
   - **Avoids exp007's failure mode.** The regression trap was driven by noisy MCTS policy
     targets and discrete-outcome value drift; TD's value-only bootstrap toward the *next
     position's equity estimate* has neither feedback loop. Still gate checkpoints on arena
     evals and keep LR conservative.
   - **Design notes.** Consider multi-component value outputs (win/gammon/bg probabilities
     combined into money equity, as TD-Gammon and GNUBG do) instead of the scalar head — gammon
     rates are a large share of the remaining gap. Capacity is not the lever: the historical
     record (GNUBG, wildbg) improved via inputs and target quality, not bigger nets.
   - **Why not the alternatives:** deeper-ply distillation costs ~53× per label (3-ply vs
     2-ply, exp009 Phase A ply pricing) for a teacher we intend to outgrow; a TPU-scale
     AlphaZero rerun needs batched-inference re-engineering and remains unproven for
     backgammon, where shallow search on small nets has been world-class for 20 years.

10. **Infrastructure.** Future experiments should be added to the `EXPERIMENTS` dict; download logs from GCS and re-render.
...
- **Resolved (2026-07-06, exp008): Yes.** On-distribution GNUBG DAgger improves on the seed (+0.48 ppg / 63-37 by round 15) and narrows the external gap to −1.19 ppg / 17% wins — the project's best result. See [Pretraining Analysis § Stage 7](pretraining_analysis.qmd) for the full write-up.
- **Superseded (2026-07-06, exp008):** The α=0 MCTS-Q variant was not needed — exp008 uses GNUBG oracle money equity directly (pure supervised distillation), so discrete-outcome noise never enters.
