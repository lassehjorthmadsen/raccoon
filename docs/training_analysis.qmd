...
5. **_MCTS_ self-play destroys a strong supervised seed.** MCTS under a GNUBG-losing prior generates targets weaker than the seed's teacher, and discrete outcome targets decalibrate a money-equity value head; 50 iterations erased a +1.98 ppg advantage ([exp007](#exp007)). Improving on a strong seed requires keeping a stronger-than-self signal in the loop — or a self-play update that does not decalibrate the value head, which is exactly what the [exp010 postscript](#exp010) finds: *TD* self-play lifts the same class of seed rather than wrecking it.
...
## Postscript — exp010: does self-play *have* to wreck a seed? {#exp010}

This section is a later coda, not part of the exp000–007 sequence: a different self-play method (TD, not MCTS) applied to a different starting point (the distilled seed the [pretraining track](pretraining_analysis.qmd) produced). It is placed here because it directly revisits Conclusion 5.
...
**Interpretation.** TD(λ) self-play is validated as a direction — the first method here to lift a strong seed via self-play — but on this recipe it stops ~0.3 short of even 0-ply parity, far from surpassing it. Reaching then surpassing 0-ply likely needs one of: a less greedy recipe (the run was pure argmax; temperature/exploration is untested), a stronger 0-ply starting point (distill *to* 0-ply parity first, then TD from there — the seed's −2.30 start capped the ceiling), or the value-head changes tracked in the [pretraining track's open questions](pretraining_analysis.qmd#open-questions). The eval-metric episode also re-confirms conclusion 4 with force: **a self-play loop must be scored against a fixed external reference, never its own seed.**
